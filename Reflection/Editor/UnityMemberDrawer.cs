...
using EnumEx = CippSharp.Reflection.Internal.Extensions;
using UnityObject = UnityEngine.Object;

namespace CippSharpEditor.Reflection
{
    [CustomPropertyDrawer(typeof(UnityMemberBase), true)]
    public class UnityMemberDrawer : TargetedDrawer
    {
        ...
        private List<DropdownOption<UnityMemberBase>> GetAllMemberOptions()
        {
        ...
        protected DropdownOption<UnityMemberBase> GetMemberOption(MemberInfo member, string component, bool inherited)
        {
            UnityMemberBase value;
            string label;
            ...
            return new DropdownOption<UnityMemberBase>(value, label);
        }
        ...
        protected virtual BindingFlags validBindingFlags
        {
            get
            {
                ...
                if (filter.Static && filter.Inherited)
        ...
        protected virtual bool ValidateMember(MemberInfo member)
        {
            ...
            { // Member is a property
                ...
                if (!filter.ReadOnly || (!filter.Properties && filter.Settable))
                    valid &= property.CanWrite;
                if (!filter.WriteOnly || (!filter.Properties && filter.Gettable))
            ...
            { // Member is a method
                ...
                if (!filter.Methods && filter.Gettable)
                    valid &= method.ReturnType != typeof(void);
